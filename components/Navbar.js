// components/Navbar.js - Fixed with Fan role support
...
export default function Navbar({ user, userProfile, onProfileUpdate }) {
  ...
  useEffect(() => {
    if (user && userProfile && userProfile.role !== 'fan') {
      fetchMyTeams();
    }
  }, [userProfile, user]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-2">
            {userProfile?.role === 'coach' ? (
              <>
                ...
                <Link
                  href="/fundraisers"
                  className={`px-4 py-2 rounded-lg transition ${isActive('/fundraisers')
                    ? 'bg-white/20 text-white backdrop-blur-sm'
                    : 'text-white/90 hover:bg-white/10'
                    }`}
                >
              ...
              <>
                ...
                <Link
                  href="/my-fundraisers"
                  className={`px-4 py-2 rounded-lg transition whitespace-nowrap ${isActive('/my-fundraisers')
                    ? 'bg-white/20 text-white backdrop-blur-sm'
                    : 'text-white/90 hover:bg-white/10'
                    }`}
                >
              ...
              <>
                <Link
                  href="/my-pledges"
                  className={`px-4 py-2 rounded-lg transition ${isActive('/my-pledges')
                    ? 'bg-white/20 text-white backdrop-blur-sm'
                    : 'text-white/90 hover:bg-white/10'
                    }`}
                >
            ...
            {/* Leaderboard - for players and coaches only */}
            {userProfile?.role !== 'fan' && (
              <Link
                href="/leaderboard"
            ...
            {/* Help Dropdown */}
            <div className="relative group">
              <button className={`px-4 py-2 rounded-lg transition flex items-center gap-1 ${isInGroup(['/help', '/about'])
                ? 'bg-white/20 text-white backdrop-blur-sm'
                : 'text-white/90 hover:bg-white/10'
                }`}>
                Help
              ...
              <div className="absolute right-0 mt-2 w-48 bg-gray-800 border border-gray-700 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                <Link
                  href="/help"
                  className={`block px-4 py-3 hover:bg-gray-700 transition ${isActive('/help') ? 'bg-blue-900/30 text-blue-400' : 'text-white'
                    } first:rounded-t-lg`}
                >
                  📚 Getting Started
                </Link>
                <Link
                  href="/help#videos"
                  className={`block px-4 py-3 hover:bg-gray-700 transition text-white`}
                >
                  🎥 Video Tutorials
                </Link>
                <a
                ...
                <Link
                  href="/about"
                  className={`block px-4 py-3 hover:bg-gray-700 transition ${isActive('/about') ? 'bg-blue-900/30 text-blue-400' : 'text-white'
                    } last:rounded-b-lg`}
                >
                  ℹ️ About RepQuest
                </Link>
          ...
          <div className="hidden md:flex items-center gap-4">
            {myTeams.length > 1 && userProfile?.role !== 'fan' && (
              <div className="relative group">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden pb-4 space-y-2">
                  ...
                  {sortedTeams.map(team => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => handleTeamSwitch(team.id)}
                        disabled={switchingTeam}
                        className={`w-full text-left px-4 py-2 rounded-lg transition ${team.id === userProfile.active_team_id
                          ? 'bg-blue-900/30 text-blue-400'
                          : 'text-white hover:bg-gray-700'
                          } ${isOutOfSeason ? 'opacity-60' : ''}`}
                      >
                        {team.id === userProfile.active_team_id && '✓ '}
                        {team.name}
                        {isOutOfSeason && <span className="text-xs ml-2 text-gray-400">(Ended)</span>}
            ...
            {userProfile?.role === 'coach' ? (
              <>
                ...
                <Link
                  href="/fundraisers"
                  onClick={() => setMobileMenuOpen(false)}
                  className={`block px-4 py-3 rounded-lg transition ${isActive('/fundraisers')
                    ? 'bg-white/20 text-white backdrop-blur-sm'
                    : 'text-white/90 hover:bg-white/10'
                    }`}
                >
              ...
              <>
                ...
                <Link
                  href="/my-fundraisers"
                  onClick={() => setMobileMenuOpen(false)}
                  className={`block px-4 py-3 rounded-lg transition ${isActive('/my-fundraisers')
                    ? 'bg-white/20 text-white backdrop-blur-sm'
                    : 'text-white/90 hover:bg-white/10'
                    }`}
                >
            ...
            <Link
              href="/profile"
              onClick={() => setMobileMenuOpen(false)}
              className="px-4 py-3 bg-white/10 backdrop-blur-sm rounded-lg block"
            >
              ...
              {activeTeam && userProfile?.role !== 'fan' && (
